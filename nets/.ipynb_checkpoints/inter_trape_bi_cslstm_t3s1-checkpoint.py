...
#         reuse = bool(gen_images)
    ...
    for t in range(seq_length//2):
        ...
        with tf.variable_scope('merge_l4', reuse=tf.AUTO_REUSE):
            if t < (seq_length//2//2):
                hiddenConcat = tf.concat([tm_hidden_fw[2][t*2], tm_hidden_bw[2][(seq_length//2-t-1)*2]], axis=-1)
                hiddenConcatConv_l4[t] = tf.layers.conv2d(inputs=hiddenConcat,
                                                          ...
                                                          padding='same',
                                                          name='F_h_merge_l4')
                memConcat = tf.concat([tm_mem_fw[2][t*2], tm_mem_bw[2][(seq_length//2-t-1)*2]], axis=-1)
                ...
                hiddenConcat = tf.concat([tm_hidden_bw[2][(seq_length//2-t-1)*2], tm_hidden_fw[2][t*2]], axis=-1)
                hiddenConcatConv_l4[t] = tf.layers.conv2d(inputs=hiddenConcat,
                                                           filters=tm_hidden_fw[2][t].get_shape()[-1],
                                                           kernel_size=1,
                                                           strides=1,
                                                           padding='same',
                                                           name='B_h_merge_l4')
                memConcat = tf.concat([tm_mem_bw[2][(seq_length//2-t-1)*2], tm_mem_fw[2][t*2]], axis=-1)
                memConcatConv_l4[t] = tf.layers.conv2d(inputs=memConcat,
                                                       filters=tm_mem_fw[2][t].get_shape()[-1],
                                                       kernel_size=1,
                                                       strides=1,
