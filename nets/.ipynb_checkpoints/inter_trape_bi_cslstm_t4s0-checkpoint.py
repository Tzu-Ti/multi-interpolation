...
#         reuse = bool(gen_images)
    ...
    hiddenConcatConv = [None for i in range(seq_length)]
    x_gen = [None for i in range(seq_length)]
    for t in range(seq_length):
        with tf.variable_scope('bi_merge', reuse=tf.AUTO_REUSE):
            if t < (seq_length//2):
                hiddenConcat = tf.concat([tm_hidden_fw[3][t], tm_hidden_bw[3][seq_length-t-1]], axis=-1)
                ...
                hiddenConcat = tf.concat([tm_hidden_bw[3][seq_length-t-1], tm_hidden_fw[3][t]], axis=-1)
                hiddenConcatConv[t] = tf.layers.conv2d(inputs=hiddenConcat,
    ...
    l2Loss = tf.nn.l2_loss(gen_images - images[:, :])
    l1Loss = tf.losses.absolute_difference(gen_images, images[:, :])
    gdlLoss = cal_gdl(gen_images, images[:, :])
