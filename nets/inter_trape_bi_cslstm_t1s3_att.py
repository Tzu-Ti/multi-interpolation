...
import numpy as np
from layers.ResGroup import resGroup
...
#         reuse = bool(gen_images)
    ...
    # hidden state to resgroup
    print("Input resblock shape: {}".format(np.shape(hiddenConcatConv[0])))
    resgroups = [None for i in range(seq_length//2)]
    for t in range(seq_length//2):
        with tf.variable_scope('resgroup', reuse=tf.AUTO_REUSE):
            group = resGroup(12, hiddenConcatConv[t].get_shape()[-1])
            tm_group = group(hiddenConcatConv[t])
            print("group return: {}".format(np.shape(tm_group)))
            resgroups[t] = tm_group
    
    for t in range(seq_length//2):
        with tf.variable_scope('generate', reuse=tf.AUTO_REUSE):
            x_gen[t] = tf.layers.conv2d(inputs=resgroups[t],
