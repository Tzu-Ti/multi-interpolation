...
class B_jstlstmCell():
    ...
    def __call__(self, x, h, c, m):
        ...
        with tf.variable_scope(self.layer_name):
            ...
            i_m, g_m, f_m = tf.split(m_cc, 3, 3)

            if x is None:
                i = tf.sigmoid(i_h + i_c + i_m)
                f = tf.sigmoid(f_h + f_c + f_m + self._forget_bias)
                g = tf.tanh(g_h + g_c + g_m)
            else:
                ...
                i = tf.sigmoid(i_x + i_h + i_c + i_m)
                f = tf.sigmoid(f_x + f_h + f_c + f_m + self._forget_bias)
                g = tf.tanh(g_x + g_h + g_c + g_m)
